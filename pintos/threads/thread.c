...
/* List of processess in THREAD_BLOCKED state, that is, processes
        that are really actually sleeping. */
...
void thread_wakeup(void)
{
    ...
    while (!list_empty(&sleep_list))
    {
        ...
        struct thread *front_thread =
            list_entry(front_thread_elem, struct thread, elem);

        if (front_thread->wakeup_tick == min_tick)
        {
...
/*
tf (interrupt frame, 인터럽트 프레임)에 저장된 정보를 바탕으로 CPU의
레지스터들을 복원하고, 최종적으로 iretq 명령어를 사용하여 CPU의 실행 흐름을
사용자 모드로 전환하고 사용자 프로그램의 특정 지점으로 점프 (user stack의 주소로
돌려놓음)
*/
...
static void schedule(void)
{
    ...
    if (curr != next)
    {
        /* If the thread we switched from is dying, destroy its struct
                 ...
                 The real destruction logic will be called at the beginning of
           the schedule(). */
